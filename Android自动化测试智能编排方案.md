...
本方案实现的 Agent 具备完整的 **感知(Perceive) → 思考(Think) → 决策(Decide) → 执行(Act)** 循环架构，并通过 **Loop 回环** 实现持续推理与执行，直到任务完成或达到最大循环次数。

### 1. 正确 ReAct + Memory 架构
...
| 模块 | 职责 | 输入 | 输出 |
|------|------|------|------|
| **Perceive (感知)** | 理解用户自然语言输入，感知当前环境状态 | 用户指令/环境信息 | 解析后的任务目标 + 当前状态 |
| **Think (思考)** | 分析当前状态，理解任务目标，识别问题 | Memory上下文 + 任务目标 | 推理结果 + 下一步计划 |
| **Decide (决策)** | 根据推理结果，决定行动方案 | 推理结果 | 具体行动指令 + 备选方案 |
| **Act (执行)** | 调用MCP工具执行操作 | 行动指令 | 执行结果 |
| **Loop (循环)** | 判断是否继续执行 | 执行结果 | 继续循环/结束任务 |
...
```
Step 1: PERCEIVE 感知 (一次性)
├── 理解用户自然语言："启动鹿客管家App，找到哈哈哈设备，进入直播"
├── 解析任务目标：启动应用 → 验证页面 → 查找设备 → 点击入口 → 验证直播
└── 输出：结构化任务描述

Step 2-N: LOOP 循环 (重复执行)
│
├── THINK 思考
│   ├── 分析当前页面状态
│   ├── 检查历史记忆
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
Step 1: PERCEIVE 感知 (一次性理解)
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

【Perceive 感知】
输入: 用户自然语言 "启动鹿客管家App，找到哈哈哈设备，进入直播"
解析:
  - 意图: 启动App并进入直播
  - 目标步骤: 启动应用 → 验证页面 → 查找设备 → 点击入口 → 验证直播
  - 参数: package=com.lockin.loock, device_name="哈哈哈"
输出: 结构化任务 {task: "直播启动流程", steps: [...]}

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
Loop 1: Think → Decide → Act → Loop判断
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

【Think 思考】
分析:
  - 第一步需要启动应用
  - 使用 package_name 启动 com.lockin.loock
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
Loop 2: Think → Decide → Act → Loop判断
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
Loop 3-5: 查找设备 "哈哈哈" 
（包含多次 Think/Decide/Act/Loop 循环处理元素定位失败）
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
决策:
  - 模式: cautious
  - 准备备选方案: textContains降级、OCR定位、坐标点击
输出: action="find_element", mode="cautious", fallback=[...]
...
【Loop 判断】
状态: 元素定位失败
分析: 可能原因 - 设备名称包含emoji("哈哈哈😂")而非纯文本
判断: 可恢复，尝试降级策略
→ 回到 THINK

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
Loop 4: Think → Decide → Act → Loop判断（降级策略）
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
【Decide 决策】
决策:
  - 降级策略：使用textContains模糊匹配
  - 模式: direct
输出: action="find_element", mode="direct", strategy="textContains"
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
Loop 5-6: Think → Decide → Act → Loop判断（点击并验证）
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
分析:
  - 已找到设备，下一步点击进入直播
  - 点击设备缩略图 ivThumb
输出: next_action="click_element", params={element: "ivThumb"}

【Decide 决策】
决策:
  - 模式: direct
输出: action="click_element", mode="direct"

【Act 执行】
调用MCP工具: mcp.click_element(resourceId="ivThumb")
等待: mcp.wait(seconds=10)
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
Loop 7: Think → Decide → Act → Loop判断（验证直播）
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
分析:
  - 需要验证直播是否启动成功
  - 检测网速信息 "KB/s"
输出: next_action="find_element", params={textContains: "KB/s"}
...
| 调试过程中的操作 | Agent模块 | 说明 |
|-----------------|----------|------|
| 理解用户指令 | Perceive | 感知解析自然语言 |
| 分析设备页面未显示原因 | Think | 思考分析可能原因 |
| 决定增加等待时间+多方式验证 | Decide | 决策采用谨慎模式 |
| 调用 start_app + dump_ui | Act | 执行MCP工具操作 |
| 判断是否继续循环 | Loop | 决策继续或结束 |
